...
def connect_database(path):
    ...
    database.execute(
        ...
        CREATE TABLE IF NOT EXISTS trips (
            ...
            requested_stop_sequence INTEGER,
    ...
    if "requested_stop_sequence" not in columns:
        database.execute("ALTER TABLE trips ADD COLUMN requested_stop_sequence INTEGER")
    if "next_stop_sequence" not in columns:
        database.execute("ALTER TABLE trips ADD COLUMN next_stop_sequence INTEGER")
    database.commit()
    return database
...
def read_trip_file(path, now, return_stop_id=False):
    ...
    with Path(path).open(newline="", encoding="utf-8") as source:
        for line_number, row in enumerate(csv.reader(source), 1):
            ...
            requested_stop_sequence = None
            if len(row) >= 4 and row[3].strip():
                try:
                    requested_stop_sequence = int(row[3].strip())
                except ValueError as exc:
                    raise ValueError(
                        f"{path}:{line_number}: invalid stop sequence {row[3]!r}"
                    ) from exc
...
def ingest_trips(database, trips):
    normalized_trips = [trip if len(trip) >= 4 else (*trip, None) for trip in trips]
    database.executemany(
        """
        INSERT INTO trips (trip_id, scheduled_at, route_id, requested_stop_sequence)
        VALUES (?, ?, ?, ?)
        ON CONFLICT(trip_id) DO UPDATE SET
            scheduled_at = excluded.scheduled_at,
            route_id = excluded.route_id,
            requested_stop_sequence = excluded.requested_stop_sequence,
            completed = 0
        """,
        [(trip_id, scheduled_at.strftime(TIME_FORMAT), route_id, stop_sequence)
         for trip_id, scheduled_at, route_id, stop_sequence in normalized_trips],
    )
...
def fetch_realtime(api_key, trip_ids, debug=False):
    if not trip_ids:
        ...
        return {}
    updates = {}
    for batch_start in range(0, len(trip_ids), REALTIME_BATCH_SIZE):
        batch = trip_ids[batch_start:batch_start + REALTIME_BATCH_SIZE]
        query = "tripid=" + ",".join(parse.quote(trip_id, safe="") for trip_id in batch)
        debug_log(debug, f"realtime query: {', '.join(batch)}")
        ...
        if not entities and len(batch) > 1:
            debug_log(debug, "realtime batch returned no entities; retrying trips individually")
            for trip_id in batch:
                updates.update(fetch_realtime(api_key, [trip_id], debug))
            continue
        for entity in entities:
            ...
            if stop_updates:
                ...
                next_stop_sequence = stop_updates[0].get("stop_sequence")
                if next_stop_sequence is not None:
                    next_stop_sequence = int(next_stop_sequence)
            ...
            debug_log(
                ...
                f"next stop={next_stop_id or 'unknown'} (sequence={next_stop_sequence or 'unknown'})",
            )
...
def debug_input_trips(trips, now, debug):
    for trip_id, scheduled, _, _ in trips:
...
def make_display_rows(database, now, updates, stop_id=None, debug=False):
    ...
    database.executemany(
                "UPDATE trips SET delay_seconds = ?, next_stop_id = ?, next_stop_sequence = ?, last_checked_at = ? WHERE trip_id = ?",
                [(update["stop_delay"] if stop_id and update["next_stop_id"] == stop_id and update["stop_delay"] is not None
                    else update["delay"], update["next_stop_id"], update.get("next_stop_sequence"),
                    now.strftime(TIME_FORMAT), trip_id)
    ...
    rows = database.execute(
        """
         SELECT trip_id, route_id, scheduled_at, delay_seconds, next_stop_id,
             requested_stop_sequence, next_stop_sequence
        FROM trips
    ...
    for row in rows:
        ...
        stops_away = None
        if row["requested_stop_sequence"] is not None and row["next_stop_sequence"] is not None:
            stops_away = row["requested_stop_sequence"] - row["next_stop_sequence"]
        debug_log(
            debug,
        ...
        if (stops_away is not None and stops_away < 0) or (
            estimated < now - timedelta(minutes=1) and not at_this_stop
        ):
        ...
        result.append({"route_id": row["route_id"], "time": estimated.strftime("%H:%M"),
                       ...
                       "delay": row["delay_seconds"], "stops_away": stops_away})
        if len(result) == 12:
            break
    return result
...
def format_csv(rows):
    lines = ["route_id,time,minutes,stops_away"]
    lines.extend(f"{row['route_id']},{row['time']},{row['minutes']},{row['stops_away'] if row.get('stops_away') is not None else ''}" for row in rows)
    return "\n".join(lines) + "\n"
...
def main():
    ...
    try:
        ...
        with connect_database(args.database) as database:
            ...
            candidate_ids = [trip_id for trip_id, _, _, _ in trips] if args.once else select_trip_ids(database, now, args.debug)
            if args.once:
